...
- (void)conventToMp3 {
    ...
    _mp3FileName = [_mp3FileName stringByAppendingString:@".mp3"];
    NSString *mp3FilePath = [[NSHomeDirectory() stringByAppendingFormat:@"/Documents/"] stringByAppendingPathComponent:_mp3FileName];
    _mp3FilePath = mp3FilePath;
    @try {
        
        int read, write;
        
        FILE *pcm = fopen([self.recordFilePath cStringUsingEncoding:NSASCIIStringEncoding], "rb");
        ...
        //一下设置要与AVAudioRecorder的设置相同
        lame_set_in_samplerate(lame, 44100);
        lame_set_VBR(lame, vbr_default);
        lame_set_num_channels(lame, 1);//声道
    ...
    @finally {
        ...
        //        [_convertFormatTimer pauseTimer];
    }
}

- (void)transformCAFToMP31 {
